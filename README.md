...
- **Use `fully-resolved` wherever `extends` is in play, for reads and writes alike.** `combined` does not expand the extends chain: an extends-based view reads the same under `combined` as under `extension`, showing the `extends:` line and any local overrides but none of the inherited fields. Only `fully-resolved` shows what the view actually provides. It is safe as a default — a fully-resolved body written back in fully-resolved mode prunes to the same stored delta a combined write would produce, and views, topics, and composite topics all accept it.
