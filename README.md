...
- **Know which layer you are reading.** `yaml-get` returns the extension layer by default — the authored deltas only. `--mode combined` adds the schema base. Neither expands `extends`.
- **Read with `fully-resolved` wherever `extends` is in play.** `combined` does not expand the extends chain: an extends-based view reads the same under `combined` as under `extension`, showing the `extends:` line and any local overrides but none of the inherited fields. Only `fully-resolved` shows what the view actually provides.
- **Fully-resolved writes are safe for top-level `extends:`, and refused in four cases.** A fully-resolved body written back in fully-resolved mode prunes to the same stored delta a combined write would produce, so it is a reasonable default. The save is rejected when the file carries `extends:` on a dimension, measure, or filter; when a topic's inline `views:` entry carries `extends:`, which is the extended-views pattern in §5; when the write removes or changes an existing `extends:` value, though adding one where none existed is allowed; or when two files in the same extends chain are saved in one batch. Use `combined` for those.
- **Do not write with `mode: merged`.** All four write modes produce the same resolved model, but `merged` stores considerably more on the branch to get there. `merged` has narrow legitimate uses — principally un-ignoring a view — none of which are migration operations. For authoring, the default is correct; select a mode deliberately or not at all.
- **Assert what the branch stores after every write.** Read back with `--mode extension` and confirm it holds only the intended delta, not a copy of content the schema layer already provides.
